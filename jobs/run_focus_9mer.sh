#!/usr/bin/env bash
# De-novo 9-mer peptide generation in the DMF5-HLA-A2 groove (6AMU/6AM5), TCR+MHC fixed motif,
# dual-interface hotspots -> ProteinMPNN unconstrained -> can we capture the 9-mer epitope MMWDRGLGM?
set -uo pipefail
cd /home/ubuntu/if-mhc
ABS=/home/ubuntu/if-mhc
source /home/ubuntu/miniforge3/etc/profile.d/conda.sh
PY=/home/ubuntu/miniforge3/envs/esmcba/bin/python
OUT=outputs/focus_9mer; mkdir -p "$OUT/bb"
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True OMP_NUM_THREADS=2
LOG="$OUT/run.log"; trap 'echo "[$(date)] EXIT $? (focus_9mer)" >>"$LOG"' EXIT
...
echo "[$(date)] RFdiffusion de-novo 9mer on 6AMU/6AM5 (DMF5 groove)" | tee "$LOG"
cd "$ABS/RFdiffusion"
while IFS=$'\t' read -r pid contig hot; do
  ls "$OUT/${pid}_0.pdb" >/dev/null 2>&1 && continue
  gpu_wait 6000
  echo "[$(date)] $pid de-novo 9mer (20 designs)" | tee -a "$ABS/$LOG"
  python run_inference.py inference.input_pdb="$ABS/inputs/focus_6am/${pid}_trim.pdb" \
     "contigmap.contigs=[$contig]" "ppi.hotspot_res=[$hot]" inference.num_designs=20 \
     inference.ckpt_override_path=models/Complex_base_ckpt.pt \
     inference.output_prefix="$ABS/$OUT/${pid}" >>"$ABS/$LOG" 2>&1
done < "$ABS/inputs/focus_6am/contigs_9mer.txt"
cd "$ABS"
...
for line in open(f"{out}/parsed.jsonl"):
    ...
    pep=next((c for c in ch if len(r.get(f"seq_chain_{c}",""))==9), ch[-1])
...
$PY focus_9mer_recovery.py "$OUT" 2>>"$LOG" | tee "$OUT/recovery_table.txt"
echo "[$(date)] DONE focus_9mer" | tee -a "$LOG"
touch "$OUT/COMPLETE"
