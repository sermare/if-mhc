...
OUT=outputs/grind_10k_T05
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True OMP_NUM_THREADS=2
LOG="$OUT/run.log"; trap 'echo "[$(date)] EXIT $? (grind_10k_T05)" >>"$LOG"' EXIT
...
echo "[$(date)] grind_10k_T05: T=0.5, 10k/crystal partitioned across backbones" | tee "$LOG"
...
PY
  mkdir -p "$cdir"
  gpu_wait 9000
  echo "[$(date)] $pid : ${NSEQ[$pid]} seq/backbone @ T=0.5" | tee -a "$LOG"
  $PY ProteinMPNN/protein_mpnn_run.py --jsonl_path "$cdir.parsed.jsonl" --chain_id_jsonl "$cdir.chain.jsonl" \
     ...
     --sampling_temp "0.5" --seed 37 --model_name v_48_020 >>"$LOG" 2>&1
