#!/usr/bin/env bash
set -uo pipefail
ABS=/home/ubuntu/if-mhc; OUT="$ABS/outputs/rfd_recover"; mkdir -p "$OUT"
...
else echo "[$(date '+%F %T')] relaunch"; TARGET="${TARGET:-15}" PAR="${PAR:-1}" HOURS="${HOURS:-12}" setsid bash "$ABS/jobs/run_recover.sh" </dev/null >>"$OUT/run.log" 2>&1 & fi
