#!/usr/bin/env python3
"""Molecular-structure figures (PyMOL) for one de-novo design vs both native registers, in the
common aligned frame (via py/align_full_pdb.py). No TCR.

Produces TWO figures:

  FIGURE A  tag "no_cog"  : el45/az180 view without cognate (cyan) peptide.
                           MHC groove cartoon+surface (surface transparency = HC_SURF_TRANSP),
                           other and design peptides shown (orange / magenta),
                           peptide N-term, C-term and the F-pocket labelled, colour legend baked
                           onto the panel.

  FIGURE B  tag "no_oth"  : el45/az180 view without other (orange) peptide.
                           MHC groove cartoon+surface (surface transparency = HC_SURF_TRANSP),
                           cognate and design peptides shown (cyan / magenta),
                           peptide N-term, C-term and the F-pocket labelled, colour legend baked
                           onto the panel.

Each view -> ray-traced PNG (with a baked colour legend) + a copy-pasteable set_view(...)
block printed and written to <base>_<tag>_views.txt / .json.

Usage (run inside the pymolviz conda env):
    python py/render_crossing_structure.py <path/to/design.pdb> [out_dir]
"""
import sys, os, subprocess
sys.path.insert(0, os.path.dirname(__file__))

ROOT = "/home/ubuntu/if-mhc"
ALIGN_DIR = f"{ROOT}/outputs/aligned"
BASE_PY = "/home/ubuntu/miniforge3/bin/python3"


def ensure_native_aligned():
    os.makedirs(ALIGN_DIR, exist_ok=True)
    for pid, fn in [("6AM5", "6AM5_native_aligned.pdb"), ("6AMU", "6AMU_native_aligned.pdb")]:
        out = f"{ALIGN_DIR}/{fn}"
        if not os.path.exists(out):
            subprocess.run([BASE_PY, f"{ROOT}/py/align_full_pdb.py",
                            f"{ROOT}/inputs/focus_6am/{pid}_trim.pdb", out], check=True)
...
def build_pymol_script(cognate_aligned, other_aligned, design_aligned, out_dir,
                       base, cognate_name, other_name, cognate_pid, other_pid):
    header = (
        'COGNATE_ALIGNED = r"%s"\n' % cognate_aligned +
        'OTHER_ALIGNED   = r"%s"\n' % other_aligned +
        'DESIGN_ALIGNED  = r"%s"\n' % design_aligned +
    ...
    return header + PYMOL_BODY


def main():
    if len(sys.argv) < 2:
        print(__doc__); sys.exit(1)
    design_path = os.path.abspath(sys.argv[1])
    base = os.path.basename(design_path).replace(".pdb", "")
    out_dir = os.path.abspath(sys.argv[2]) if len(sys.argv) > 2 \
        else f"{ROOT}/outputs/figures_lt2p1/{base}_views"
    os.makedirs(out_dir, exist_ok=True)

    ensure_native_aligned()
    design_aligned = f"{ALIGN_DIR}/{base}_aligned.pdb"
    if not os.path.exists(design_aligned):
        subprocess.run([BASE_PY, f"{ROOT}/py/align_full_pdb.py", design_path, design_aligned],
                       check=True)

    cognate_pid = "6AMU" if ("6AMU" in base or base.startswith("DRG")) else "6AM5"
    other_pid = "6AMU" if cognate_pid == "6AM5" else "6AM5"
    cognate_name = "GIG" if cognate_pid == "6AM5" else "DRG"
    other_name = "DRG" if cognate_pid == "6AM5" else "GIG"

    script = build_pymol_script(
        cognate_aligned=f"{ALIGN_DIR}/{cognate_pid}_native_aligned.pdb",
        other_aligned=f"{ALIGN_DIR}/{other_pid}_native_aligned.pdb",
        design_aligned=design_aligned, out_dir=out_dir, base=base,
        cognate_name=cognate_name, other_name=other_name,
        cognate_pid=cognate_pid, other_pid=other_pid,
    )
    script_path = f"{out_dir}/{base}_render_views.py"
